...
const TrustedByBox = ({ trusted }) => {
 const handleDelete = async () => {
  await axios.delete(`${SITE_URL}wp-json/ws-api/v1/trust/delete?trust_id=${trusted.trusted_id}`).then((res) => {
   if (res.status === 200) {
    location.reload()
   }
  })
 ...
 return (
  <div className="flex items-center justify-between gap-4 py-2 my-1 border-y border-y-main-primary">
   <div className="relative overflow-hidden rounded-md">
    <img src={trusted.trusted_img} className="object-cover w-20" alt="" />
   </div>
