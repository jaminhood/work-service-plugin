...
const WhyChooseUsCard = ({ benefit }) => {
 const handleDelete = async () => {
  await axios.delete(`${SITE_URL}wp-json/ws-api/v1/reason/delete?benefit_id=${benefit.benefit_id}`).then((res) => {
   if (res.status === 200) {
    location.reload()
   }
  })
 ...
 return (
  <div className="flex gap-4 py-2 my-1 border-y border-y-main-primary">
   <div className="relative w-[calc(100%-5rem)]">
    <h4 className="text-base font-sansation-bold">{benefit.benefit_heading}</h4>
    <p className="text-sm font-jost-regular opacity-70">{benefit.benefit_paragraph}</p>
   </div>
